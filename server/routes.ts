...
Rules:
- Extract every vehicle mentioned in the text
- Use empty string "" for missing values
- For condition: use "used" or "new"
- For drivetrain: use "fwd", "rwd", "awd", or "4wd"
- For price and kilometers: numbers only (no currency symbols or units)
...
Return ONLY the CSV data, nothing else:`;
      ...
      if (!csvContent) {
        return res.status(500).json({ error: "No CSV data generated by Claude" });
      }

      res.json({ csv: csvContent.trim() });
    } catch (error) {
